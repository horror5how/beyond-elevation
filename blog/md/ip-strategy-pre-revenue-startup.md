...
The distinction from general [startup IP strategy](/blog/posts/ip-strategy-startups-guide/) is timing. A post-revenue startup can afford to retroactively audit and file. A pre-revenue startup must build protection into the founding architecture. The cost of getting it right at this stage is measured in thousands. The cost of retrofitting later is measured in millions — if the window has not already closed.
...
**Step 3: Trade secret classification (Week 3).** Classify internal assets that should not be patented: training data, proprietary datasets, system prompts, hyperparameters, internal benchmarks, customer-specific configurations. Implement access controls, confidentiality agreements, and a documented classification protocol. An unclassified trade secret is not a trade secret — it is [institutional knowledge](/blog/posts/trade-secrets-vs-patents-strategy-guide/) that walks out when your first engineer leaves.
...
**Mistake 4: Filing one broad patent instead of a cluster.** A single patent is a target. A [patent cluster](/blog/posts/patent-clustering-strategy-moat/) — 5 to 10 related filings covering the core method plus adjacent implementations — is a fortress. Clusters are harder to design around, more valuable in licensing negotiations, and more persuasive to investors. Hayat Amin's rule is direct: "One patent is a lottery ticket. Seven patents around the same innovation is a moat."
...
**Book a pre-seed IP audit at [beyondelevation.com](https://beyondelevation.com)** and run the Pre-Seed IP Filing Sequence before your next investor conversation. The filing windows close whether you are ready or not.
...
[Book the diagnostic →](https://usemotion.com/meet/hayat-amin/be?ref=blog-ip-strategy-pre-revenue-startup)
