...
Hayat Amin has developed what Beyond Elevation calls the **Pool Rate Challenge Audit** — a structured assessment that quantifies an implementer's leverage position before engaging a pool operator. The audit scores five factors: the ratio of actually practised SEPs to total pool patents, available comparable bilateral [FRAND rates](/blog/posts/sep-licensing-frand-explained/), the pool's injunction track record, the implementer's volume-based negotiating weight, and the jurisdictional landscape for FRAND determination.
...
Hayat Amin reminds founders that Beyond Elevation's advisory practice has seen implementers reduce pool exposure by 15 to 40 percent simply by identifying SEPs in the pool that are expired, not practised, or not essential to the standard. The [royalty benchmarking data](/blog/posts/patent-royalty-rates-founders-underprice/) confirms that most pool rates embed a significant premium over what bilateral FRAND rates would produce for the same technology.
...
Hayat Amin's rule for SEP holders is blunt: the [licensing revenue model](/blog/posts/patent-licensing-revenue-model/) must evolve. The era of fixed, non-negotiable pool rates is ending regardless of how this case is decided. Smart SEP holders are already building bilateral optionality so they are not trapped when the pool reprices.
...
[Book the diagnostic →](/call/web?ref=blog-patent-pool-court-set-royalty-implementer-2026)
