#!/usr/bin/env node
// Post a video to TikTok via Browserbase residential session + Playwright.
// Usage: node post.mjs <video.mp4> "<caption>" <cookies.json>
import { chromium } from 'playwright';
import { readFileSync } from 'node:fs';

const VIDEO = process.argv[2];
const CAPTION = process.argv[3];
const COOKIES_PATH = process.argv[4];

const cookies = JSON.parse(readFileSync(COOKIES_PATH, 'utf8')).map(c => ({
  name: c.name,
  value: c.value,
...
if (!BB_KEY || !BB_PROJECT) throw new Error('Browserbase env missing');

console.log('[tt] creating Browserbase residential session…');
    ...
    await page.evaluate(() => {
      ...
      dialogs.forEach(d => {
        ...
        for (const b of btns) {
          ...
          if (labels.some(l => t === l || t.startsWith(l))) { try { b.click(); return; } catch {} }
        }
        const close = d.querySelector('[data-e2e="modal-close-inner-button"], [aria-label*="lose" i]');
        if (close) try { close.click(); } catch {}
      });
      document.querySelectorAll('div.TUXModal-overlay, [data-floating-ui-portal]').forEach(el => el.style.pointerEvents = 'none');
    });
  } catch {}
...
} catch (e) { console.log('[tt] caption fail:', e.message.split('\n')[0]); }
...
for (let i = 0; i < 8 && !posted; i++) {
  ...
  console.log(`[tt] attempt ${i+1}:`, JSON.stringify(ok));
  if (ok.clicked && !ok.disabled) {
    ...
    if (ok2 || page.url().includes('content') || page.url().includes('manage')) { posted = true; break; }
  }
  await page.waitForTimeout(2000);
}
